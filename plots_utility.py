...
def plt_doppler_antennas(doppler_spectrum_list, sliding_lenght, delta_v, name_plot):
    if doppler_spectrum_list:
        ...
        for p_i in range(len(doppler_spectrum_list)):
            ...
            ax1.format_coord = lambda x, y: f"Temps = {x * sliding_lenght * 6e-3:.2f} s  |  Vitesse = {(y - length_v) * delta_v:.2f} m/s" # 
        ...
        for axi in ax:
            axi.label_outer()
        fig.set_size_inches(20, 10)
        plt.locator_params(axis='x', nbins=20) # for not having too much values on X-axis 
        plt.title('Spectrogramme Doppler')
        plt.xlabel('Temps')
        plt.ylabel('Vitesse (m/s)')
        plt.savefig(name_plot, bbox_inches='tight')
        print(name_plot, os.path.abspath(name_plot))
        plt.close()
